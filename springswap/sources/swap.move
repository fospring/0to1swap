module spring_swap::swap {

    use aptos_framework::coin::{Coin, value};
    use aptos_framework::coin;
    use spring_swap::lp_coin::Lp;
    use spring_swap::lp_coin;
    use std::signer;
    ...
    public fun create_pool<X, Y>(sender: &signer) {
        assert!(signer::address_of(sender) == @spring_swap,ERROR_MUST_ADMIN);
    ...
    acquires Pool {
        let pool = borrow_global_mut<Pool<X, Y>>(@spring_swap);
    ...
    public fun is_exists<X, Y>(): bool {
        exists<Pool<X, Y>>(@spring_swap)
    }


    /// usdc -> usdt
    public fun swap_x_to_y<In, Out>(coinx: Coin<In>): Coin<Out>acquires Pool {
        let pool = borrow_global_mut<Pool<In, Out>>(@spring_swap);
    ...
    public fun swap_y_to_x<In, Out>(coin_in: Coin<In>): Coin<Out>acquires Pool {
        let pool = borrow_global_mut<Pool<Out, In>>(@spring_swap);
